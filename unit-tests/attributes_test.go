...
func Test_Attributes_Default(t *testing.T) {
	...
	// Assert
	assert.Assert(t).That(attribs.Foreground(), is.EqualTo(color.Ignore), "Foreground")
	assert.Assert(t).That(attribs.Background(), is.EqualTo(color.Ignore), "Background")
}
...
func Test_Attributes_ModifyDefault(t *testing.T) {
	...
	newAttributes, changed := original.Modify(addition)
...
func Test_Attributes_ModifyResetNoColour(t *testing.T) {
	...
	addition := ansi.NewAttributeBuilder().
		WithBackground(color.Reset).
		Build()
	_, changed := original.Modify(addition)
